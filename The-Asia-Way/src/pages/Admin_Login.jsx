...
const Admin_Login = () => {
    ...
    const handleSubmit = (e) => { e.preventDefault(); console.log("Admin Login:", { username, password }); navigate("/admin/home"); };
    const inputStyle = (field) => ({ ...s.input, borderColor: focused === field ? "#C2410C" : "#E8D5B7" });

    return (
        <div style={s.page}>
            <div style={s.card}>
                <div style={s.iconWrap}><ShieldCheck size={24} color="#C2410C" /></div>
                <div style={s.title}>Admin Login</div>
                <div style={s.subtitle}>Restricted access — authorised personnel only</div>
